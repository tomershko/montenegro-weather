...
const origin = env('POSTCARD_ORIGIN') || 'https://tomershko.github.io';
...
function validateDetails(data){
  const limits={recipient:80,place:80,date:10,message:600,sender:80};const clean={};
  for(const [key,limit]of Object.entries(limits)){if(typeof data?.[key]!=='string'||!data[key].trim()||data[key].length>limit)throw new Error('details');clean[key]=data[key].trim();}
...
export async function handler(req){
  ...
  try{
    ...
    if(action==='create'&&req.method==='POST'){
      ...
      if(!(photo instanceof Blob)||photo.type!=='image/jpeg'||photo.size<4||photo.size>2097152)return json({error:'invalid_photo'},400);
      const bytes=new Uint8Array(await photo.arrayBuffer());
      if(bytes[0]!==255||bytes[1]!==216||bytes[2]!==255||bytes[bytes.length-2]!==255||bytes[bytes.length-1]!==217)return json({error:'invalid_photo'},400);
    ...
    if(!row.ready||Date.parse(row.expires_at)<=Date.now())return json({error:'expired'},410);
    if(action==='view')return json({...row.details,expiresAt:row.expires_at,serverNow:new Date().toISOString()});
    const image=await service('/storage/v1/object/authenticated/postcards/'+tokenHash+'.jpg');
    const body=await image.arrayBuffer();
    if(Date.parse(row.expires_at)<=Date.now())return json({error:'expired'},410);
    return new Response(body,{headers:{...headers,'Content-Type':'image/jpeg'}});
  }catch(e){return json({error:e.quota?'quota':'service_unavailable'},e.quota?429:503);}
}
Deno.serve(handler);
